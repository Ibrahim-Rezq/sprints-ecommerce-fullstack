          ...
          <table className='table table-striped table-bordered table-sm'>
            <thead>
              <tr>
                {values.map((value) => {
                  return (
                    <th key={value} className='p-3' scope='col'>
                      {value}
                    </th>
                  );
                })}
            ...
            <tbody>
              {ordersArray.map((order) => {
                const { id, number, cost, orderData, statues } = order;

                return (
                  <tr key={id}>
                    <td style={{ minWidth: '7rem' }}>{number}</td>
                    <td style={{ minWidth: '7rem' }}>{cost}</td>
                    <td style={{ minWidth: '7rem' }}>{orderData}</td>
                    <td style={{ minWidth: '7rem' }}>{statues}</td>
